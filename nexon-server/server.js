  ...
  if (mytextpass != "" && mytextpass != undefined) {
    bcrypt.genSalt(saltRounds, function (err, salt) {
      ...
      bcrypt.hash(mytextpass, salt, function (err, hash) {
        ...
        //쿼리작성
        conn.query(
          `insert into member(m_id,m_name, m_pass,m_passch,m_email,m_phone,m_address1,m_address2)
...
app.post("/findid", async (req, res) => {
  ...
  conn.query(
    `select * from member where m_name = '${m_name}' and m_phone='${m_phone}' and m_email='${m_email}'`,
    (err, result, fields) => {
      if (result) {
  ...
  if (mytextpass != "" && mytextpass != undefined) {
    bcrypt.genSalt(saltRounds, function (err, salt) {
      ...
      bcrypt.hash(mytextpass, salt, function (err, hash) {
        ...
        //쿼리작성
        conn.query(
          `update member set m_pass ='${myPass}', m_passch='${myPass}' where m_id='${m_id}'
                `,
          (err, result, fields) => {
            if (result) {
              res.send("등록되었습니다.");
...
app.post("/news", async (req, res) => {
  const { n_title, n_date, n_titledesc, n_desc, n_image, n_category } =
    req.body;
  conn.query(
